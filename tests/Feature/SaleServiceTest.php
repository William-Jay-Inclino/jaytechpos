...
it('generates unique invoice numbers per user', function () {
    ...
    expect($invoice1)->toBe('INV-'.date('Y').'-00001');
...
it('increments invoice numbers correctly for the same user', function () {
    ...
    Sale::factory()->create([
        'user_id' => $user->id,
        'invoice_number' => "INV-{$currentYear}-00001",
    ]);

    Sale::factory()->create([
        'user_id' => $user->id,
        'invoice_number' => "INV-{$currentYear}-00003",
    ]);

    // Generate next invoice number - should be 0004 (max + 1)
    $nextInvoice = $this->saleService->generateInvoiceNumber($user->id);
    expect($nextInvoice)->toBe("INV-{$currentYear}-00004");
});
...
it('handles year rollover correctly', function () {
    ...
    Sale::factory()->create([
        'user_id' => $user->id,
        'invoice_number' => "INV-{$lastYear}-00005",
    ]);
    ...
    expect($currentYearInvoice)->toBe("INV-{$currentYear}-00001");
});
